		...
		<div class="donation-summary-wrapper has-background-bright columns has-padding-18">
			<div class="column is-half">
				<membership-summary :address="confirmationData.address"
					...
					<div class="has-margin-top-18" v-if="hasIncentive()">{{ $t( 'membership_confirmation_success_text_incentive' ) }}</div>
					<div class="has-margin-top-18" v-if="!hasIncentive()">{{ $t( 'membership_confirmation_success_text' ) }}</div>
				</membership-summary>
			</div>
...
export default Vue.extend( {
	...
	methods: {
		hasIncentive(): boolean {
			return this.confirmationData.membershipApplication.incentive !== undefined
				&& this.confirmationData.membershipApplication.incentive !== '';
		},
	},
} );
</script>

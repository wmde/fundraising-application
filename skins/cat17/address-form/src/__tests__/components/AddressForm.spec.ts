...
describe('AddressForm.vue', () => {
	...
	it('updates fieldErrors correctly according to the validation result', () => {
		...
		if (comp !== undefined) {
			if (typeof comp.fieldErrors === 'function') {
				let errors = comp.fieldErrors();
				expect(errors.postcode).toBe(false);
			}
		}
	});

	it('displays errors for the correct fields', () => {
		const wrapper = mount(AddressForm, {
			store,
			localVue,
			propsData: newTestProperties({ isCompany: false })
		...
		if (comp !== undefined) {
			if (typeof comp.fieldErrors === 'function') {
				...
				expect(errors.lastName).toBe(false);
				expect(errors.street).toBe(false);
				expect(errors.postcode).toBe(false);
				expect(errors.city).toBe(true);
				expect(errors.country).toBe(false);
			}
		}
	});
